...
//#include <gl\gl.h>			pulled via Model_3DS.h
...
//////////////////////////////////////////////////////////////////////
// making the library more cross-platform happy
//////////////////////////////////////////////////////////////////////

//NOTES FOR PORTING from http://the-labs.com/Blender/3DS-details.html
//SOME CODE BORROWED FROM http://www.faqs.org/faqs/graphics/fileformats-faq/part4/section-6.html
// short is always a two byte integer
// long is always a four byte integer
// float is always a four byte IEEE floating point numebr
// char is a single byte integer

static void fread_ushort(unsigned short &s,FILE* fp) {
	s = (unsigned short) (fgetc(fp) & 0xFF);
	s |= ((unsigned short) (fgetc(fp) & 0xFF) << 0x08);
}

static void fread_ulong(unsigned long &l, FILE *fp) {
	l = (unsigned long) (fgetc(fp) & 0xFF);
	l |= ((unsigned long) (fgetc(fp) & 0xFF) << 0x08);
	l |= ((unsigned long) (fgetc(fp) & 0xFF) << 0x10);
	l |= ((unsigned long) (fgetc(fp) & 0xFF) << 0x18);
}

static void fread_float(float &f, FILE *fp) {
	unsigned long l;
	fread_ulong(l,fp);
	float *hack;
	hack = (float *) & l;
...
//more "complicated" data types, just to put in one place
void fread_header(struct Model_3DS::ChunkHeader &h,FILE* fp) {
	fread_ushort(h.id,fp);
	fread_ulong(h.len,fp);
}
...
if (lit) glEnable(GL_LIGHTING);
			...
			} else for (int j = 0; j < Objects[i].numMatFaces; j ++)
			{
...
void Model_3DS::FloatColorChunkProcessor(long length, long findex, int matindex)
{
	...
	fread_float(r,bin3ds);
	fread_float(g,bin3ds);
	fread_float(b,bin3ds);
...
void Model_3DS::VertexListChunkProcessor(long length, long findex, int objindex)
{
	...
	{
		fread_float(tmp,bin3ds); Objects[objindex].Vertexes[i]=tmp;
		fread_float(tmp,bin3ds); Objects[objindex].Vertexes[i+2]=tmp;
		fread_float(tmp,bin3ds); Objects[objindex].Vertexes[i+1]=tmp;
...
void Model_3DS::TexCoordsChunkProcessor(long length, long findex, int objindex)
{
	...
	{
		fread_float(tmp,bin3ds); Objects[objindex].TexCoords[i] = tmp;
		fread_float(tmp,bin3ds); Objects[objindex].TexCoords[i+1] = tmp;
	}
...
void Model_3DS::FacesDescriptionChunkProcessor(long length, long findex, int objindex)
{
	...
	for (int i = 0; i < numFaces * 3; i+=3)
	{
		...
		fread_ushort(vertA,bin3ds);
		fread_ushort(vertB,bin3ds);
		fread_ushort(vertC,bin3ds);
		fread_ushort(flags,bin3ds);
...
void Model_3DS::FacesMaterialsListChunkProcessor(long length, long findex, int objindex, int subfacesindex)
{
	...
	fread_ushort(numEntries,bin3ds);
